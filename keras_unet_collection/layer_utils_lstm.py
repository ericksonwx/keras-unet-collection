...
This was a copy of the layer_utils.py, but adapted to include 2dconvlstm.
    Adapted by Randy J. Chase 

Note the expected shape of the convlstm layer is [None, time, x, y, channel]
"""
...
from tensorflow.keras.layers import MaxPooling3D, AveragePooling3D, UpSampling3D, Conv2DTranspose, GlobalAveragePooling2D
from tensorflow.keras.layers import ConvLSTM2D, DepthwiseConv2D, Lambda
...
def encode_layer(X, channel, pool_size, pool, kernel_size='auto', l1=1e-2, l2=1e-2,
    ...
    if pool == 'max':
        ...
        X = AveragePooling2D(pool_size=pool_size, name='{}_avepool'.format(name))(X)
        ...
        X = ConvLSTM2D(channel, kernel_size, strides=(pool_size, pool_size), 
                   kernel_regularizer=regularizers.L1L2(l1=l1, l2=l2),
...
def CONV_stack(X, channel, kernel_size=3, stack_num=2, dilation_rate=1, l1=1e-2, l2=1e-2, activation='ReLU',batch_norm=False, 
                    kernel_initializer='glorot_uniform',name='conv_stack',return_sequences=True):
    '''
    ...
    for i in range(stack_num):
        ...
        # linear convolution
        X = ConvLSTM2D(channel, kernel_size, padding='same', use_bias=bias_flag, 
                   kernel_regularizer=regularizers.L1L2(l1=l1, l2=l2), return_sequences=return_sequences,
...
def CONV_output(X, n_labels, kernel_size=1, l1=1e-2, l2=1e-2, activation='Softmax',kernel_initializer='glorot_uniform',
    ...
    '''
    #by default will not return a sequence since we just want the new forecast 
    X = ConvLSTM2D(n_labels, kernel_size, padding='same', use_bias=True, kernel_initializer=kernel_initializer,
